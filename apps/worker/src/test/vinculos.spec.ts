...
beforeAll(() => {
  const setupSql = `
    ...
    CREATE TABLE vinculos_funcionais (id text PRIMARY KEY NOT NULL, membro_id text NOT NULL, funcao_id text NOT NULL, regional_id text, administracao_id text, setor_id text, casa_id text, grupo_trabalho_id text, ativo integer DEFAULT true NOT NULL, created_at text DEFAULT CURRENT_TIMESTAMP NOT NULL, updated_at text DEFAULT CURRENT_TIMESTAMP NOT NULL, FOREIGN KEY (membro_id) REFERENCES membros(id), FOREIGN KEY (funcao_id) REFERENCES funcoes(id), FOREIGN KEY (regional_id) REFERENCES regionais(id), FOREIGN KEY (administracao_id) REFERENCES administracoes(id), FOREIGN KEY (setor_id) REFERENCES setores(id), FOREIGN KEY (casa_id) REFERENCES casas(id), FOREIGN KEY (grupo_trabalho_id) REFERENCES grupos_trabalho(id),
    ...
    CREATE UNIQUE INDEX idx_vinculo_unico_regional ON vinculos_funcionais (membro_id, funcao_id, regional_id) WHERE regional_id IS NOT NULL AND ativo = 1;
    CREATE UNIQUE INDEX idx_vinculo_unico_administracao ON vinculos_funcionais (membro_id, funcao_id, administracao_id) WHERE administracao_id IS NOT NULL AND ativo = 1;
    CREATE UNIQUE INDEX idx_vinculo_unico_setor ON vinculos_funcionais (membro_id, funcao_id, setor_id) WHERE setor_id IS NOT NULL AND ativo = 1;
    CREATE UNIQUE INDEX idx_vinculo_unico_casa ON vinculos_funcionais (membro_id, funcao_id, casa_id) WHERE casa_id IS NOT NULL AND ativo = 1;
    CREATE UNIQUE INDEX idx_vinculo_unico_gt ON vinculos_funcionais (membro_id, funcao_id, grupo_trabalho_id) WHERE grupo_trabalho_id IS NOT NULL AND ativo = 1;
  `
...
describe('Testes de Vínculos Funcionais', () => {
  let membroId = 'membro-123'
  let funcaoId1 = 'funcao-1'
  let funcaoId2 = 'funcao-2'
  let regId = 'reg-1'
  let admId = 'adm-1'
  let setorId = 'setor-1'
  let casaId = 'casa-1'
  ...
  beforeAll(() => {
    sqlite.exec(`
      INSERT INTO regionais (id, nome) VALUES ('${regId}', 'Regional');
      INSERT INTO administracoes (id, regional_id, nome) VALUES ('${admId}', '${regId}', 'Adm');
      INSERT INTO setores (id, administracao_id, nome) VALUES ('${setorId}', '${admId}', 'Setor');
      INSERT INTO casas (id, setor_id, nome) VALUES ('${casaId}', '${setorId}', 'Casa 1');
      INSERT INTO casas (id, setor_id, nome) VALUES ('${casaId2}', '${setorId}', 'Casa 2');
      INSERT INTO grupos_trabalho (id, nome, regional_id) VALUES ('${gtId}', 'GT 1', '${regId}');
      
      INSERT INTO membros (id, nome, casa_id) VALUES ('${membroId}', 'Pessoa Teste', '${casaId}');
      INSERT INTO funcoes (id, nome) VALUES ('${funcaoId1}', 'Responsável');
      INSERT INTO funcoes (id, nome) VALUES ('${funcaoId2}', 'Membro');
    `)
  })
  ...
  it('7. criar vínculo Regional', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, regionalId: regId })
    })
  ...
  it('8. criar vínculo Administração', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, administracaoId: admId })
    })
    expect(res.status).toBe(201)
  })
  ...
  it('9. criar vínculo Setor', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, setorId })
    })
    expect(res.status).toBe(201)
  })
  ...
  it('10. criar vínculo Casa', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, casaId })
    })
  ...
  it('11. criar vínculo GT', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, grupoTrabalhoId: gtId })
    })
    expect(res.status).toBe(201)
  })
  ...
  it('13. permitir mesma função para duas Casas diferentes', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId1, casaId: casaId2 })
    })
    expect(res.status).toBe(201)
  })
  ...
  it('15. rejeitar vínculo sem escopo', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId2 })
    })
    expect(res.status).toBe(400)
  })
  ...
  it('16. rejeitar vínculo com dois escopos', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId2, casaId, setorId })
    })
    expect(res.status).toBe(400)
  })
  ...
  it('17. rejeitar vínculo com três ou mais escopos', async () => {
    const res = await req('/api/v1/vinculos-funcionais', {
      method: 'POST',
      body: JSON.stringify({ membroId, funcaoId: funcaoId2, casaId, setorId, regionalId: regId })
    })
    expect(res.status).toBe(400)
  })
  ...
  it('14 b. Deve permitir recriar vínculo exato se o anterior estiver inativo', async () => {
    ...
    expect(res.status).toBe(201) // O anterior foi inativado no teste 21, então deve permitir
  })

  it('2. Validação de estado resultante no PATCH (não pode ter dois escopos)', async () => {
    const res = await req(`/api/v1/vinculos-funcionais/${vinculoCasaId}`, {
      method: 'PATCH',
      body: JSON.stringify({ setorId }) // tenta adicionar setorId num vínculo que já tem casaId
    })
    expect(res.status).toBe(400)
    const json = await res.json()
