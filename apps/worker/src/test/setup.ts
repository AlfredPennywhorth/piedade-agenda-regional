export function setupDb(sqlite: any) {
  const setupSql = `
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_checkin_unico ON checkins (evento_id, membro_id) WHERE ativo = 1;
    CREATE INDEX IF NOT EXISTS idx_checkins_evento_id ON checkins (evento_id);
    CREATE INDEX IF NOT EXISTS idx_checkins_membro_id ON checkins (membro_id);
    CREATE UNIQUE INDEX IF NOT EXISTS idx_checkin_token_hash ON checkin_tokens (token_hash);
    CREATE INDEX IF NOT EXISTS idx_checkin_token_evento_membro ON checkin_tokens (evento_id, membro_id);
  `
